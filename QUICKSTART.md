...
|------------|---------|------------|--------|
| ACR1281U-C | 13.56 MHz | Только ЕКП | ✅ Работает |
| IQRFID-5102 #1 | UHF 900 MHz | Библ. карты (идентификация) | ⚠️ Тест протокола 0xA0 |
| IQRFID-5102 #2 | UHF 900 MHz | Книжные метки (выдача) | ❗ Нужно купить |
| RRU9816 | UHF 900 MHz | Книжные метки | ✅ **Драйвер готов!** |

**✅ RRU9816 теперь работает на RPi!** Протокол реверснут, DLL не нужна.
```bash
...
4. **Протокол IQRFID-5102:** заголовок 0xA0, checksum (~SUM+1)&0xFF
5. **Протокол RRU9816:** заголовок = длина пакета, CRC-16 в конце
